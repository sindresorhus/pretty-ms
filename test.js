...
function runTests({title, defaultOptions, cases}) {
	test(title, t => {
		...
		for (const testCase of cases) {
			...
			t.is(format(milliseconds, options), expected, String(milliseconds));

			// const bigint = toBigInt(milliseconds);
			// if (typeof bigint === 'bigint') {
			// 	t.is(format(bigint, options), expected, String(milliseconds) + 'n');
			// }
		}
	});
}
...
runTests({
	...
	cases: [
		...
		[Number.MAX_VALUE, ' 218d 8h 8m 48s'],
	],
});
...
runTests({
	...
	cases: [
		...
		[Number.MAX_VALUE, ':218:08:08:48'],
	],
});
